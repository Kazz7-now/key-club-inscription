...
def admin():
    ...
    conn = get_db()
    days = conn.execute("""
        SELECT g.name AS game_name, d.label, d.event_date, gd.capacity,
               gd.start_time, gd.end_time,
        ...
        JOIN event_days d ON d.id = gd.day_id
        LEFT JOIN registrations r ON r.game_day_id = gd.id
        GROUP BY gd.id ORDER BY d.event_date, g.id
    """).fetchall()
    ...
    """).fetchall()
    conn.close()
    return render_template("admin.html", days=days, registrations=registrations)
